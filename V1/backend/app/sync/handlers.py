...
def sync_articoli(session: Session) -> int:
    ...
    try:
        rows = fetch_easyjob("""
            SELECT ART_COD, ART_DES1, CAT_ART6
            FROM ANAART
        ...
        for row in rows:
            ...
            existing = session.execute(
                text("SELECT id FROM articoli WHERE codice = :c"),
                {"c": codice},
            ).fetchone()
                ...
                session.execute(
                    text("""
                        INSERT INTO articoli
                            (id, codice, codice_upper, descrizione, categoria, synced_at)
                        VALUES
                            (:id, :c, :cu, :des, :cat, :ts)
                    """),
                    {
